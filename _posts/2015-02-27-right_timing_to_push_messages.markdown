...
유저의 접속 세션 데이터를 기반으로, 개별 유저에게 적절 푸시 메시지 타이밍을 결정한다.

# 문제 제기

모바일 앱이 많아지면서 유저의 재방문을 높이기 위해 푸시 메시지를 사용하는 서비스가 많다.
문제는 유저를 고려하지 않는 푸시 메시지인데, 스팸으로 취급될 뿐더러 서비스 이탈을 가속화시키기도 한다.
만약 유저의 접속 패턴을 고려해서 적당한 푸시 타이밍에 메시지를 보낼 수 있다면, 마케팅 효과를 극대하면서 과도한 푸시 메시지의 부작용을 피할 수 있다.

# 기본 아이디어

푸시 메시지로 달성하려는 목적은 다양한데, 이 글에서는 잘 접속하던 유저의 접속 주기가 증가하는 경향을 보일 때, 이를 관리하기 위한 목적으로 보내는 푸시 메시지를 가정한다.
이미 확실하게 이탈한 유저를 다시 모집하거나, 별 다른 목적 없이 마케팅 비용을 소진시킬 목적의 푸시 메시지는 논의에서 제외한다.
...
서비스 사용 빈도가 줄어드는 유저의 활동을 다시 활성화시키려는 목적의 푸시 메시지를 보내기 위해서, 유저의 기존 접속 패턴을 사용한다.

유저의 접속 패턴을 관찰하면, 특정 시간대에 접속이 집중되는 것을 알 수 있다.
일반적으로 점심 시간과 저녁 퇴근 시간대, 저녁 식사 시간 이후부터 잠자기 전까지의 시간대에서 피크를 보인다.
이는 매우 일반적인 현상이며, 대부분의 유저들에게서 공통적으로 관찰할 수 있는 접속 패턴이다.

그러나 이러한 현상은 모든 유저들을 하나의 그룹으로 합쳐서 보았을 때 나타나는 현상이며, 유저 개개인의 사정은 모두 다르다.
즉, 평균적인 유저의 접속 패턴은 전체 집단의 그것과 유사하지만, 유저를 더 자세하게 관찰하면 주로 접속하는 시간은 유저마다 상당히 다르다.
이러한 개별 유저의 접속 세션을 이용하여 개별 유저의 접속 시간 분포를 만들 수 있다.

# 개별 유저의 접속 시간 분포

접속 세션은 로그인 시간에서 시작하여 로그아웃 시간으로 끝나는, 일종의 가로로 긴 형태의 막대기라고 할 수 있다.
만약 세션이 로그인 시간과 로그아웃 시간에 따라 가로 길이가 결정되고, 그 세로 길이는 일정한 찰흙 막대기라면, 이 막대기를 바닥에 차곡차곡 쌓을 수 있다.
바닥에는 '시각(time)'이라는 눈금이 있어서 찰흙 막대기를 이 눈금에 맞춰서 정확한 자리에 쌓는다.

![session_bar](/img/posts/2015-02-27-right_timing_to_push_messages/session_bar.png)

유저의 세션 막대는 시간이 갈수록 여러 개가 생긴다.
한 명의 유저가 기록하는 모든 세션 정보를 새로운 막대기에 비유하면 아주 많은 막대기를 얻을 수 있다.
이렇게 얻은 개별 유저의 세션 막대들을 아래 그림처럼 동일한 시간 축에 계속 차곡차곡 쌓는다.
이는 유저 전체에 대한 세션 막대기가 아니라, 특정 유저 한 명만의 정보다.
만약 100명의 유저가 있다면 100개의 서로 다른 시간 축에 개별 유저의 세션 막대기만 따로 쌓아야 한다.

![session_bar2](/img/posts/2015-02-27-right_timing_to_push_messages/session_bar2.png)

만약 아래에 이미 쌓인 찰흙 막대기가 있다면, 그 위에 추가로 세션 찰흙을 쌓는다.
...
유저 개인의 수 많은 세션을 이렇게 쌓으면 이는 특정 유저 개인의 접속 시간 분포가 된다.
총 넓이가 1이 되도록 표준화시키면 이를 통해 특정 유저가 어느 시간대에 접속해 있을지 확률을 계산할 수 있다.

![session_bar4](/img/posts/2015-02-27-right_timing_to_push_messages/session_bar4.png)

접속 분포 함수를 $$f$$라고 한다면 $t_1$ 시점부터 $t_2$ 시점까지 유저가 접속해 있을 확률은 아래와 같이 구할 수 있다. 

$$ P(t_1 \le X \le t_2) = \int_{t_1}^{t_2}f(x)dx$$

A라는 유저의 세션 정보로 위의 접속 시간 분포를 만들었다면, 유저 A가 오전 11시 30분에서 오후 1시 사이에 접속해 있을 확률은 아래와 같이 계산 가능하다.
...
유저의 접속 패턴은 보통 요일에 따라 다른 양상을 보이며, 특히 평일과 휴일이 크게 다르다.
위의 예시에서는 요일 패턴을 감안하지 않았으며, 편의를 위해 접속 시간을 시간 단위로만 기록하였다.
실제 데이터를 사용하게 되면, 분 단위의 시간 축으로 표현하며, 평일 여부에 따라 다른 분포가 나타난다.

# 접속 시간 분포를 활용한 푸시 타이밍

모든 유저에 대해서 접속 시간 분포를 알고 있다면, 가장 접속 가능성이 높은 시간대를 찾을 수 있다.
해당 시간대는 각각의 유저가 서비스에 주로 접속했던 시간대이므로, 푸시 메시지를 받고 무언가 행동을 취할 가능성이 높다.
예를 들어, 평균 접속 주기를 지나서도 접속이 없는 유저에게 재접속을 유도하는 푸시 메시지를 보낸다면, 해당 서비스를 주로 사용하던 시간대에 메시지를 보내는 것이 합리적이다.

거의 모든 서비스가 유저의 세션 정보를 기록하여 보관한다.
주어진 상황에서 알고 있는 정보를 최대한 활용해, 가장 좋은 의사 결정을 하는 것이 데이터 분석의 목적이다.
이미 알고 있는 유저의 접속 정보를 활용해서 푸시 메시지를 보낼 시간을 정하는 것은 현명한 의사결정 방법이다.

다만, 유저의 세션 정보에서 아직 활용하지 않은 데이터가 있는데, 바로 현 시점에서 유저의 접속 여부다.
...
이런 확률을 계산하기 위해서 조건부 확률을 이용하는 것도 좋은 방법인데, 이를 위해서는 세션 데이터를 더 가공해야 한다.
...
해당 시간에 유저가 접속할 확률은 $33\,/\,34 \approx 97.06\%$다.
아직 유저가 접속할 기회는 많다.
...
그래도 앞으로 유저가 접속할 가능성은 $28\,/\,34 \approx 82.35 \%$다.
아직 기다릴 수 있다. 유저에게 연락하지 않는다.

![session_bar_t1](/img/posts/2015-02-27-right_timing_to_push_messages/session_bar_t1.png)

시간이 더 흘러서 $t_2$가 되었다.
점심 시간 직전인데, 아직 유저는 접속하지 않았으며, 유저가 접속할 가능성은 더 줄어들었다.
앞으로 유저가 접속할 가능성은 $21\,/\,34 \approx 61.76 \%$다.
약간 불안하다. 
오늘 유저를 만나지 못할 수도 있을 것 같다.
그러나 아직 연락하지 않고 참는다.
...
앞으로 유저가 접속할 가능성은 $12\,/\,34 \approx 35.29\%$다.
오늘 접속하지 않을 가능성이 $50\%$이하로 떨어졌으니, 당장 유저에게 푸시 메시지를 보낸다.
"지금 들어오시면, 선물을 드립니다."
...
$t_{3-1}$로 돌아가서 12시 30분의 상황을 살펴보면, 유저가 접속할 가능성은 $16\,/\,34 \approx 47.06\%$다.
이미 유저가 오늘 접속할 가능성은 절반 이하다.
...
유저들의 접속 여부를 준실시간으로 확인해서 확률을 구해야 한다.
...
웹이나 앱의 접속 기록에서 로그아웃 기록이 없는 경우가 많다. 이러한 경우, 특정 기준을 사용하여 적절한 로그아웃 시간을 만들어주어야 한다.
...
이런 경우, 실제로 유저에게 취할 액션과 전략을 먼저 결정하고, 필요한 정보만 최소화하여 기록한다.
...
확률에 가치(돈)을 부여해서 기대 가치를 계산하고, 이를 기준으로 실행 자원을 사용하는 것이 중요하다.

## 컨텐츠를 고려한 메시지 내용

타이밍과 함께 중요한 것이 메시지의 내용이다.
여러가지 방안이 있지만, 유저가 최근에 경험하고 있는 컨텐츠와 관련이 있는 메시지를 구성할 수 있다면 효과적이다.
만약, 유저 경험의 허들 구간이 존재한다면 허들 부근의 유저에게는 좋은 타이밍에 허들 관련 메시지를 보내는 것을 고려할 수 있다. 
아무리 메시지 타이밍을 잘 선택하더라도, 메시지의 내용이 부실하다면 원하는 행동을 이끌어낼 수 없다.

## A/B 테스트
푸시 메시지 규모를 나누어 A/B 테스트를 하면 효과를 확실하게 알 수 있다.
정말 유저 접속 분포를 사용해서 메시지를 보내면 반응률이 더 높은지 실험을 통해 검증한다.
위에서 정리한 내용은 개별 상황에 따라서 그 결과가 다를 수 있다.
일반적인 내용이므로 실제 서비스에 적용한다면 테스트가 필요하다.
극단적으로는 '그냥 점심 시간에 일괄적으로 보내는 것과 차이가 없다'는 결론에 도달할 수도 있다.
아주 작은 규모라도 테스트를 반드시 수행할 것을 추천한다. 
