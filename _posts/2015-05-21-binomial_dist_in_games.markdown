...
확률 분포(distribution)에서 ditribute은 총합 1인 확률을 나눠준다는 의미다.
...
이를 위해서 먼저 강화에 영향을 줄 수 있는 요소들을 살펴본다.
...
아래와 같이 정하면, 기본 1,000원의 비용이 들어가고, +7강부터 1,500원, +10강 이상부터 2,000원의 금액이 들어간다.
...
일단 +6강 모의 실험과 비교해서 비용이 크게 증가한 것을 알 수 있다.
...
## +10강에 도전하는 10,000명의 플레이어

드디어 +10강이다.
게임마다 상징적인 강화 수치가 있기 마련인데, 이번 모의 실험에서는 +10강이 그렇다.
강화 실패 페널티로 장비가 사라질 수 있기 때문에 강화 시도에 들어가는 실질 비용은 매우 크다.
...
절반의 플레이어는 약 360,000원을 사용하여 +10강에 성공하였다.
가장 운이 나쁘지만 의지가 강한 플레이어는 약 5,000,000원을 사용하여 +10강 무기를 만들었다.
95%의 플레이어가 +10강 장비를 갖기 위해서는 약 1,500,000원의 비용이 들어간다.

![sim_cumrate_n10000_l10](/img/posts/2015-05-21-binomial_dist_in_games/sim_cumrate_n10000_l10.png)

비용 분포는 오른쪽 꼬리가 매우 길어진 형태다.
운이 정말 안 좋은 플레이어는 500만원이 없으면 +10강 무기를 못 만든다.
그나마 성공할 확률은 10% 정도다.
플레이어가 접하는 장비 강화 경험은 이렇게 나타난다.
이는 애초에 설정한 초기 값에 의해서 결정된다.
플레이어 경험을 미리 가늠해보고 신중하게 정할 필요가 있다.
